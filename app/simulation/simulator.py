...
try:
    ...
    logger.warning(f"Invalid timezone in settings: {settings.timezone}. Defaulting to UTC.")
    TIMEZONE = pytz.UTC
...
def send_with_retry(url, payload, retries=3, backoff_factor=2):
    """
    Send data with retry logic for transient failures using the requests library.
    """
